...
The movie hash remains the first request. Its positive file size is sent as `moviebytesize` when known, while an unknown size never suppresses an otherwise valid hash request. A hash answer containing a true `moviehash_match` is authoritative and skips the weaker text request. Empty or heuristic hash answers fall back to the normalized text query, then merge and deduplicate with hash results first. The request normalizer still sorts parameters by name, lower-cases with `Locale.ROOT`, and leaves spaces for the caller's canonical `+` encoding. Opaque generated/camera/messaging IDs and names made only of technical tags do not auto-fill a low-signal text title, but their file hash may still search them. No search is sent until the user presses Search; at that point only the normalized query, valid structured fields, selected language, and optional hash/size are sent to OpenSubtitles under the existing online-subtitle privacy contract.
