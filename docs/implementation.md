...
TV and paired-phone renaming share one ordinary single-line Compose text field, so Android TV Gboard supplies both the on-screen keyboard and its built-in voice input; Flick requests no microphone permission and runs no speech recognizer. The editor selects the current name, uses a text keyboard with word capitalization and IME Done, and asks the TV IME for centred placement. Input and saved names share the 80-Unicode-code-point wire ceiling, and the cap uses code-point offsets so a surrogate pair is never split. Blank canonical input cannot save; Cancel and Back do not write. Opening the editor from Pair first closes that authorization surface, then reopens it on exit, so neither a live code nor an on-TV confirmation can be hidden under the modal. A successful TV rename updates the durable preference, the visible state, and the active NSD registration. A paired-phone save calls `PairingManager.rename`, whose record rewrite preserves key id, key, pairing date, last-device tracking, and any live authenticated session.
