...
| `delayMs` | JSON integer `-5000..5000` inclusive **and** a multiple of `25` |
...
`setAudioDelay` is the live A/V sync nudge, and its field set is exactly `t,v,castId,delayMs` — nothing else. **A positive `delayMs` means the audio is heard LATER than the picture; a negative one means it is heard EARLIER; `0` is in sync.** `delayMs` outside `-5000..5000`, off the 25 ms step, or not a JSON integer is a malformed frame and is refused exactly as an out-of-range `setVolume` level is — never clamped or rounded into a value the phone did not ask for. The bound is **legality, not renderability**, and the distinction is load-bearing. A delay claims `|delay|` of forward buffer whichever way it points, and how much forward buffer exists is a property of the heap the receiver's process was granted: `BufferBudgetPolicy` yields 14,297 ms of 100 Mbps content on a 512 MB grant, 7,301 ms at 256 MB, 2,738 ms at 96 MB, and 2,282 ms on its 32 MiB byte floor, and the receiver's cap reserves the load control's own resume threshold on top of that — 9,532 / 4,868 / 1,826 / 1,522 ms. So only the 512 MB tier renders the full five seconds; a 256 MB TV clamps a full-range offset by 132 ms and the two smallest clamp hard. The wire range is the widest any receiver accepts; a receiver that cannot carry the value it was sent **clamps what it renders** against its own budget and says so in its log. It does not refuse — a socket that died because of how much heap that particular TV was granted would be a far worse failure than a delay that stopped short, and the refusal rule exists to catch malformed frames, not small devices. The range is **not** part of the `audio-delay` capability entry — the verb and its field set are unchanged — so a version-skewed pair still handshakes and a phone that has been widened alone will have a frame past the old bound refused, at the cost of the socket. A frame naming a cast that is not current returns `commandRejected(stale_cast)`, like every other cast command.
