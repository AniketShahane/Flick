...
The QR encodes `flick://pair?v=4&h=<tv-lan-ip>&p=<port>&c=<code>` — **the live four-digit code included** — while `v=3` (`h`/`p` only) and the bare `v=2` still parse as launch-only envelopes. One scan therefore completes pairing, and on that path the code is **not** a second factor.

The two ingresses are deliberately not equivalent. `PairLaunch.parseScanned` is the only one that may retain a code, because this app's own camera read the string and the QR was physically in front of the user. Every externally delivered launch — an Intent from any installed app, or a browser hand-off — goes through `PairLaunch.parse`, which **demotes a v4 payload to a v3 one inside the parser and drops the four digits before anything downstream can be offered them**; `PairLaunchParseResult.Valid` has no field able to carry a code, so an Intent cannot deliver one at all. On that path the typed code remains the sole authorization factor. A payload wrong in any field is rejected whole rather than salvaged into a prefill, since a malformed v4 is not a v3. For an Intent arrival Flick synchronously clears the incoming intent data before publishing an ephemeral event, then prefills host and port and focuses the code cell. Host and port from either ingress stay an untrusted hint — like an unpaired NSD result — until a code proves the endpoint: the phone never dials a scanned address blind.

What bounds the scanned path is the code's own life, and **not** the cumulative failure ceiling described below, which charges only *wrong* codes and so cannot bound a code that was simply read. The code is authorized only while the TV's pairing surface is visibly rendered; a TV that already has a phone paired renders no surface at all; and it is single-use with a five-minute TTL. Anyone who can read or photograph that screen while it is up holds enough to pair. What that grants is "cast to this TV", not read access to the owner's files.
