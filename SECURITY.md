...
The only accepted QR URI is `flick://pair?v=2`. It carries no endpoint, identity, code, key, nonce, proof, or capability. The phone camera launches Flick, which synchronously clears the incoming intent data before publishing an ephemeral event and opens an empty form. The user independently types the TV's visible host, port, and four-digit code; neither the custom URI nor an unpaired NSD result can supply them.
